...
export default function DialogCrearMedico({ open, onClose }) {
  ...
  const crearMedico = useCrearMedico({
    onSuccess: (data) => {
      if (
        ...
        form.especialidadesSeleccionadas.forEach((especialidadId) => {
          asignarEspecialidad.mutate({
            medicoId: data.id,
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      telefono,
      direccion,
      descripcion: descripcion?.trim() || "Médico general registrado",
      imagen,
        ...
        <div className="grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select onValueChange={(val) => handleChange("tipoDocumentoId", val)}>
            ...
            <div>
              ...
              <Input value={form.numeroDocumento} onChange={(e) => handleChange("numeroDocumento", e.target.value)} />
            </div>
          </div>
          ...
          <div>
            ...
            <Input value={form.numeroColegiatura} onChange={(e) => handleChange("numeroColegiatura", e.target.value)} />
          </div>

          {/* Tipo Médico */}
          <div>
            <Label>Tipo Médico</Label>
            <Select value={form.tipoMedico} onValueChange={(val) => handleChange("tipoMedico", val)}>
          ...
          {/* RNE y Especialidades solo para ESPECIALISTA */}
          {form.tipoMedico === "ESPECIALISTA" && (
            <>
              <div>
                <Label>RNE</Label>
                <Input value={form.numeroRNE} onChange={(e) => handleChange("numeroRNE", e.target.value)} />
              </div>
              <div>
                <Label>Especialidades</Label>
                <Select multiple onValueChange={(vals) => handleChange("especialidadesSeleccionadas", vals)} value={form.especialidadesSeleccionadas}>
                  <SelectTrigger><SelectValue placeholder="Selecciona una o más" /></SelectTrigger>
                  <SelectContent>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <Button onClick={handleSubmit} disabled={crearMedico.isPending}>Guardar</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
